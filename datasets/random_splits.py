...
from Fewshot.AllDataloader import MyDataSet, MultiClassLoader
...
def get_datasets(ds_type, num_classes=2):
    if ds_type == 'all':
    ...
    elif ds_type == "multiclass":
        dl = MultiClassLoader(bs=0, num_rows=0, num_targets=0, num_classes=num_classes, ds_names=-1)
        all_datasets = dl.all_datasets
        all_data_names = [str(d) for d in all_datasets]
...
if __name__ == "__main__":
    all_data_names = get_datasets("multiclass", 3)
    ...
    with open("./datasets/grouped_datasets/3class_split", "w") as fp:
      toml.dump(splits, fp) 
