...
for ds in datasets:
    ...
    for model in model_names:
        try:
            m, std = get_accuracy(model, ds_name=ds, num_rows=15, num_cols=-3)
            ds_result[model] = m
...
with open("./Results/old_final/6.1/raw.pkl", "rb") as f:
...
with open("./Results/old_final/12/raw.pkl", "rb") as f:
    results = pickle.load(f)
    results = results[results["model"].isin(["FLAT", "FLAT_maml"])]
    s2 = results.drop("num_cols", axis=1)
    # s2 = r.drop("model", axis=1)

with open("./Results/old_final/13/raw.pkl", "rb") as f:
    results = pickle.load(f)
    results = results[results["model"].isin(["FLAT", "FLAT_maml"])]
    s3 = results.drop("num_cols", axis=1)
    # s3 = r.drop("model", axis=1)

with open("./Results/old_final/14/raw.pkl", "rb") as f:
    results = pickle.load(f)
    results = results[results["model"].isin(["FLAT", "FLAT_maml"])]
