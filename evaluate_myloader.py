...
num_targets = 4 # cfg["num_targets"]
num_cols = cfg.get("num_cols")
shuffle_cols = cfg.get("shuffle_cols", True)

model = ModelHolder(all_cfgs)
...
def get_batch(dl):
    ...
    return None, xs_meta, xs_target, ys_meta, ys_target
...
def get_baseline_accuracy(model, xs_meta, ys_meta, xs_target, ys_target):
    ...
    a = ys_lr_target_labels == np.array(ys_target)
    print(a)
    # exit(3)
    accuracy = a.sum().item() / len(ys_target)
    return accuracy
...
baseline_models = [LogisticRegression(max_iter=1000), SVC(), RandomForestClassifier(n_estimators=30)]
baseline_model_names = ['LR', 'SVC', 'RF']

seen_data_names = ["statlog-heart", "horse-colic", "fertility", "post-operative",]# cfg["train_data_names"] # Datasets used during training
...
for num_cols in num_cols_ls:
    seen_acc = []
    unseen_acc = []
    baseline_acc = {n: [] for n in baseline_model_names}
